...
function updateProgressUI() {
    if (audio && currentData && !isNaN(audio.currentTime)) {
        ...
        if (currentTimeLabel) currentTimeLabel.innerText = toTime(currentSec);
        if (durationLabel) durationLabel.innerText = currentData.duration;
    }
}
...
function updateDisplay(data) {
    ...
    if (durationLabel) durationLabel.innerText = data.duration;
    
    let now = (audio && !isNaN(audio.currentTime)) ? audio.currentTime : toSeconds(data.current);
    currentTimeSpan.innerText = toTime(now);
    if (currentTimeLabel) currentTimeLabel.innerText = toTime(now);
    
    let progress = toSeconds(data.duration) > 0 ? (now / toSeconds(data.duration)) * 100 : 0;
    ...
    playBtn.onclick = () => {
        if (audio && currentData && currentData.url) {
            if (isMuted) {
                ...
                isMuted = false;
                muteBtn.innerHTML = `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
...
function setupSettings() {
    ...
    let settingsPanel = document.getElementById('settingsContent');
    let closeBtn = document.getElementById('closeSettings');
    ...
    document.querySelectorAll('.glass-copy').forEach(btn => {
        btn.onclick = () => {
